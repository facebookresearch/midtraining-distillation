...
class LoggingArgs:
    ...
    routing_freq: Optional[int] = (
        10  # Log detailed routing every routing_freq optimizer steps
    )
...
class MetricLogger:
    ...
    def open(self):
        ...
        if (
            ...
            wandb_kwargs = {
                k: v
                for k, v in asdict(self.args.logging.wandb).items()
                if v is not None
            }
            try:
                run = wandb.init(
                ...
                print(
                    f"WARNING: wandb.init() failed ({e}). Continuing without wandb logging.",
                    flush=True,
                )
