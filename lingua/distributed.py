...
default_no_recompute_ops = {
    ...
    # torch.ops.xformers.efficient_attention_forward_cutlass,  # Commented out due to xformers compatibility issues
}
...
def dist_mean(x: Union[int, float], mesh: DeviceMesh = None):
    tensor = torch.tensor(x).cuda()
    # Only do all_reduce if distributed is initialized
    if dist.is_initialized():
        dist.all_reduce(
            tensor, op=ReduceOp.AVG, group=mesh.get_group() if mesh else None
        )
    return tensor
...
def get_is_slurm_job() -> bool:
    # Must have SLURM_JOB_ID and SLURM_NTASKS (set by srun) to be a distributed SLURM job
    return (
...
def setup_env(env_args):
    ...
    # We change the tmp dir to a node-local scratch path in case it's a slurm job
    ...
    if get_is_slurm_job():
        # Prefer the cluster-provided SLURM_TMPDIR (set by most slurm setups);
        # fall back to /scratch/slurm_tmpdir/<jobid> for sites that use it,
        # and finally to the system tempdir.
        new_tmp = os.environ.get("SLURM_TMPDIR")
        if not new_tmp:
            fallback = f"/scratch/slurm_tmpdir/{os.environ.get('SLURM_JOB_ID', '')}"
            new_tmp = fallback if os.path.exists(fallback) else tempfile.gettempdir()
...
def setup_torch_distributed(dist_args):
    ...
    logger.info("=" * 60)
    logger.info("DISTRIBUTED DEBUG INFO:")
    logger.info(f"  get_is_torch_run(): {get_is_torch_run()}")
    logger.info(f"  get_is_slurm_job(): {get_is_slurm_job()}")
    logger.info(f"  SLURM_JOB_ID: {os.environ.get('SLURM_JOB_ID', 'NOT SET')}")
    logger.info(
        f"  SLURM_JOB_NODELIST: {os.environ.get('SLURM_JOB_NODELIST', 'NOT SET')}"
    )
    logger.info(f"  SLURM_PROCID: {os.environ.get('SLURM_PROCID', 'NOT SET')}")
    logger.info(f"  SLURM_NTASKS: {os.environ.get('SLURM_NTASKS', 'NOT SET')}")
    logger.info(f"  LOCAL_RANK env: {os.environ.get('LOCAL_RANK', 'NOT SET')}")
    ...
    # Only init distributed if world_size > 1 (multi-GPU/multi-node)
    # For single GPU jobs without proper SLURM_NTASKS set, skip distributed init
    if get_world_size() > 1:
        torch.distributed.init_process_group(init_method="env://", backend="nccl")
    else:
        logger.info("Skipping distributed init for single GPU job (world_size=1)")
