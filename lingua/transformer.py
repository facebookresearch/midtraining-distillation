...
# Try to import flash attention for varlen support
try:
...
class Attention(nn.Module):
    ...
    def forward(
        ...
        cu_seqlens: Optional[
            torch.Tensor
        ] = None,  # For FA2 varlen: cumulative sequence lengths
        max_seqlen: Optional[
            int
        ] = None,  # For FA2 varlen: max sequence length in batch
    ) -> torch.Tensor:
        # B S D
        ...
        if attn_impl == "fa2_varlen" and HAS_FLASH_ATTN and cu_seqlens is not None:
            # Flash Attention 2 variable-length attention for cross-document masking
            # This is highly efficient and prevents attention across document boundaries

            # FA2 expects (total_tokens, n_heads, head_dim) not (batch, seq, heads, dim)
            # For packed sequences, we flatten batch and seq dimensions
            total_tokens = bsz * seq_len

            # Reshape: (B, S, H, D) -> (B*S, H, D)
            xq = xq.view(total_tokens, self.n_heads, self.head_dim)
            xk = xk.view(total_tokens, self.n_heads, self.head_dim)
            xv = xv.view(total_tokens, self.n_heads, self.head_dim)

            # cu_seqlens should be (num_docs + 1,) with cumulative lengths
            # max_seqlen is the maximum document length in the batch
            if max_seqlen is None:
                # Compute from cu_seqlens if not provided
                doc_lens = cu_seqlens[1:] - cu_seqlens[:-1]
                max_seqlen = doc_lens.max().item()
