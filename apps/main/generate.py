...
def main():
    ...
    # Ensure 'config' key is present
    if "config" not in cli_args:
        raise ValueError("Missing required argument: config=<path_to_yaml_config>")

    # Load YAML file specified by config
    file_cfg = OmegaConf.load(cli_args.config)

    # Remove 'config' from CLI args to avoid clashing
    del cli_args.config
    ...
    consolidate_path = consolidate_checkpoints(cfg_dict["ckpt"])
    ...
    prompts = [
        "Tell me a story about a boy wizard named Harry Potter. ",
