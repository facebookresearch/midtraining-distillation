...
# Override RLVR-1 source:
#   RLVR1_CKPT=/path/to/rlvr1 sbatch post_training/scripts/train_olmo2_1b_rlvr2.sh

#SBATCH --job-name=olmo2_1b_rlvr2_4node_scaled
#SBATCH --output=${SLURM_LOG_DIR}/olmo2_1b_rlvr2_4node_scaled-%j.out
#SBATCH --error=${SLURM_LOG_DIR}/olmo2_1b_rlvr2_4node_scaled-%j.err
...
# RLVR1_CKPT resolution: two modes.
#   (a) Direct: set RLVR1_CKPT to a leaf dir with config.json (e.g. .../step_2600).
#       This overrides the auto-resolution below.
#   (b) Lazy:   set RLVR1_PARENT_DIR + RLVR1_EXP_NAME (both known at submit time).
#       At runtime we glob ${RLVR1_PARENT_DIR}/${RLVR1_EXP_NAME}__1__<ts>_checkpoints/step_*
#       and pick the most recent. This lets pipeline_post_training.sh chain
#       RLVR2 with --dependency=afterok:<rlvr1_jid> without a per-run launcher.
if [[ -z "${RLVR1_CKPT:-}" && -n "${RLVR1_PARENT_DIR:-}" && -n "${RLVR1_EXP_NAME:-}" ]]; then
    _snap=$(ls -1dt "${RLVR1_PARENT_DIR}/${RLVR1_EXP_NAME}__1__"* 2>/dev/null | head -1)
    if [[ -z "${_snap}" || ! -d "${_snap}" ]]; then
        echo "ERROR: no RLVR1 snapshot under ${RLVR1_PARENT_DIR}/${RLVR1_EXP_NAME}__1__*" >&2
        exit 1
    fi
    # Which step_N dir to take. The published recipe produces 2604 GRPO
    # updates (TOTAL_EPISODES=4000000 / (96 prompts x 16 samples) = 2604) with
    # SAVE_FREQ=200, so the last saved checkpoint is step_2600. Anyone who
    # lowers TOTAL_EPISODES for a smoke test gets a different final step, so
    # rather than hardcoding it we take the highest-numbered step dir that
    # actually exists and fail loudly if there is none.
    _ckpt_root="${_snap}_checkpoints"
    RLVR1_CKPT=$(ls -1d "${_ckpt_root}"/step_* 2>/dev/null \
                 | sed 's/.*step_//' | sort -n | tail -1 \
                 | while read -r _n; do echo "${_ckpt_root}/step_${_n}"; done)
    if [[ -z "${RLVR1_CKPT}" || ! -d "${RLVR1_CKPT}" ]]; then
        echo "ERROR: no step_* checkpoint under ${_ckpt_root}" >&2
        echo "       (RLVR1 may not have reached its first save_freq=${SAVE_FREQ:-200} boundary)" >&2
        exit 1
    fi
    echo "[rlvr2] Lazy-resolved RLVR1_CKPT=${RLVR1_CKPT}"
fi
RLVR1_CKPT="${RLVR1_CKPT:-${OUT_ROOT}/olmo2_1b_baseline28800_rlvr1_4node_scaled}"
EXP_NAME="${EXP_NAME:-olmo2_1b_baseline28800_rlvr2_4node_scaled}"
...
if [[ ! -d "${RLVR1_CKPT}" ]]; then
  echo "ERROR: RLVR1_CKPT missing: ${RLVR1_CKPT}"; exit 1
fi
...
"${PYTHON_BIN}" -u open_instruct/grpo_fast.py \
    ...
    --model_name_or_path "${RLVR1_CKPT}" \
    --tokenizer_name_or_path "${RLVR1_CKPT}" \
    --add_bos \
...
echo "=== RLVR-2 done -> ${OUTPUT_DIR} ==="

# Auto-launch the OLMES eval on the final RLVR2 checkpoint, IF a
# corresponding eval_single_rlvr2.sh wrapper exists. The existing
# eval_single_rlvr1.sh is rlvr1-specific (it hard-codes the "rlvr1" naming
# convention in some paths); rather than coerce it, we no-op when a rlvr2
# wrapper is absent. Create ${OLMES_ROOT}/eval_single_rlvr2.sh
# (sibling of eval_single_rlvr1.sh) to enable auto-eval.
# Set AUTO_EVAL=0 to opt out unconditionally.
if [ "${AUTO_EVAL:-1}" = "1" ]; then
    EVAL_WRAPPER="${EVAL_WRAPPER:-${OLMES_ROOT}/eval_single_rlvr2.sh}"
    if [ -x "${EVAL_WRAPPER}" ]; then
        ...
        echo "=== AUTO_EVAL: no rlvr2 eval wrapper at ${EVAL_WRAPPER}; skipping. ==="
        echo "    Create one (mirror of eval_single_rlvr1.sh) to enable auto-eval."
    fi
fi
